...
namespace Pong
{
    public partial class Form1 : Form
    {
        ...
        private void Timer_Tick(object sender, EventArgs e)
        {
            if (gameRuning)
            {
                ...
                if (ballX<=50&&ballX>=30&&ballY>=leftPaddleY&&ballY<=leftPaddleY+100)
                {
                    ...
                    ballX = 50;// Не даем мячу пройти сквозь границу ракетки(прямоугольника)
                }
        ...
        private void Form1_Paint(object sender, PaintEventArgs e)
        {
            ...
            if(!gameRuning)
            {
                string winner = (leftscore >= 5) ? "Левый победитель" : "Правый победил"; // записывает текст о победителе в переменную
